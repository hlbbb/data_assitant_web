...
const PythonAssessmentPage: React.FC = () => {
  const { refreshProfile } = useSmartQuiz('python')
  ...
  const loadNextQuestion = useCallback((state: typeof assessmentState) => {
    const q = pickAssessmentQuestion(state, pythonQuestions, PYTHON_BOARD_ORDER)
    setCurrentQuestion(q)
    if (!q) {
      const result = finishAssessment(state, 'python', PYTHON_BOARD_ORDER, PYTHON_LEVELS)
  ...
  const handleSelectOption = (label: string) => {
    ...
    const newState = processAssessmentAnswer(assessmentState, currentQuestion.id, isCorrect, pythonQuestions)
    setAssessmentState(newState)
  }

  const handleNext = () => {
    if (checkAssessmentEnd(assessmentState)) {
      const result = finishAssessment(assessmentState, 'python', PYTHON_BOARD_ORDER, PYTHON_LEVELS)
      setFinalLevel(result.level)
      refreshProfile()
      setPhase('result')
      return
    ...
    return (
      <div className="assessment-page">
        <div className="practice-page__nav">
          <Link to="/quiz/python" className="quiz-btn quiz-btn--ghost">&larr; 返回</Link>
          <span className="practice-page__title">Python 水平测评</span>
          <span />
        </div>
        <div className="assessment-page__intro">
          <h2>Python 水平测评</h2>
          <p>通过自适应测评，精准定位你的 Python 等级</p>
  ...
  if (phase === 'result') {
    const levelDef = PYTHON_LEVELS.find(l => l.level === finalLevel) ?? PYTHON_LEVELS[0]
    return (
      <div className="assessment-page">
        <div className="practice-result">
          <div className="practice-result__card">
            ...
            <div className="practice-result__actions" style={{ marginTop: 20 }}>
              <Link to="/quiz/python" className="quiz-btn quiz-btn--primary" style={{ textDecoration: 'none' }}>
                进入智能刷题
              </Link>
            </div>
          </div>
...
export default PythonAssessmentPage
