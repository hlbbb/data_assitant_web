...
import { sqlQuestions, CATEGORY_MAP, DIFFICULTY_MAP } from '../../data/sqlQuestions'
import { useSmartQuiz, smartPickQuestions, getLevelColor, getLevelDef, SQL_LEVELS, SQL_BOARD_ORDER } from '../../hooks/useSmartQuiz'
...
const SqlQuizPage: React.FC = () => {
  ...
  const { profile, initProfile, refreshProfile, levels } = useSmartQuiz('sql')
  const [selectedCategory, setSelectedCategory] = useState('')
  const [selectedDifficulty, setSelectedDifficulty] = useState('')
  const [searchQuery, setSearchQuery] = useState('')

  const levelColor = profile ? getLevelColor(profile.level) : '#8BC34A'
  ...
  const categoryStats = useMemo(() => {
    ...
    Object.keys(CATEGORY_MAP).forEach(cat => {
      stats[cat] = sqlQuestions.filter(q => q.category === cat).length
    })
    return stats
  }, [])
  ...
  const handleSmartPractice = () => {
    const questions = smartPickQuestions('sql', sqlQuestions, SQL_BOARD_ORDER, SQL_LEVELS, 20)
    navigate('/quiz/sql/practice', { state: { questions, mode: 'smart' } })
  }

  // 水平测评
  const handleAssessment = () => {
    navigate('/quiz/sql/assessment')
  }
  ...
  const handleStartPractice = (mode: 'all' | 'category' | 'random') => {
    ...
    navigate('/quiz/sql/practice', { state: { questions, trial: !unlocked } })
  }
              ...
              <div key={q.id} onClick={() => handleStartPractice('all')} style={{ marginBottom: '10px' }}>
                <div className="quiz-card">
                  <div className="quiz-card__header">
                    ...
                    <span className="quiz-card__cat">{CATEGORY_MAP[q.category]}</span>
                  </div>
