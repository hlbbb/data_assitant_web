...
function extractEstimatedTime(content: string): string | null {
  // 匹配 "预计X天" 或 "预计 X 小时" 等格式
  const match = content.match(/预计\s*(\d+)\s*(天|小时|分钟|周)/)
  if (match) {
    return `${match[1]} ${match[2]}`
  }
  // 匹配 "学习时长：X天" 格式
  const match2 = content.match(/学习时长[：:]\s*(\d+)\s*(天|小时|分钟|周)/)
  if (match2) {
...
const DetailPage: React.FC<DetailPageProps> = ({ type }) => {
  ...
  const estimatedTime = useMemo(() => extractEstimatedTime(mdContent), [mdContent])
        ...
        <div className="detail-page__practice-panel">
          ...
          <div className="detail-page__practice-panel-body">
            {type === 'sql' ? (
              ...
              <PythonPlayground
                key={dbKey}
                initialCode={initialCode || "# 在这里写 Python 代码\nprint('Hello, Python!')"}
              />
            )}
          </div>
        </div>
