...
/**
 * 处理 DATEDIFF 函数，支持嵌套函数作为参数
 * 例如: DATEDIFF(order_date, LAG(order_date) OVER(...))
 */
function convertDatediff(s: string): string {
  // 找到所有 DATEDIFF 调用
  const datediffRegex = /\bDATEDIFF\s*\(/gi
  let result = s
  let match: RegExpExecArray | null

  while ((match = datediffRegex.exec(result)) !== null) {
    const startIndex = match.index
    const openParenIndex = startIndex + match[0].length - 1 // '(' 的位置

    // 找到第一个参数的结束位置（逗号）
    const firstArgEnd = findArgEnd(result, openParenIndex + 1, ',')

    // 找到第二个参数的结束位置（右括号）
    const secondArgEnd = findArgEnd(result, firstArgEnd + 1, ')')

    if (firstArgEnd !== -1 && secondArgEnd !== -1) {
      const d1 = result.substring(openParenIndex + 1, firstArgEnd).trim()
      const d2 = result.substring(firstArgEnd + 1, secondArgEnd).trim()

      // 替换 DATEDIFF(...) 为 CAST(julianday(d1) - julianday(d2) AS INTEGER)
      const replacement = `CAST(julianday(${d1}) - julianday(${d2}) AS INTEGER)`
      result = result.substring(0, startIndex) + replacement + result.substring(secondArgEnd + 1)

      // 重置 regex 搜索位置
      datediffRegex.lastIndex = startIndex + replacement.length
    }
  }
 ...
 * 处理嵌套的括号
 */
function findArgEnd(s: string, start: number, endChar: string): number {
  let parenDepth = 0
  let i = start
  ...
  while (i < s.length) {
    ...
    if (char === '(') {
      ...
      if (parenDepth === 0 && endChar === ')') {
        return i
      }
      parenDepth--
    } else if (char === ',' && parenDepth === 0 && endChar === ',') {
      return i
    }
