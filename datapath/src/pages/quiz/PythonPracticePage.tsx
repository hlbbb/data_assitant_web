...
import { updateSmartWeights, PYTHON_BOARD_ORDER } from '../../hooks/useSmartQuiz'
import { useQuiz } from '../../hooks/useQuiz'
import { formatContent } from '../../utils/formatContent'
import OptionCard from '../../components/quiz/OptionCard'
import QuizProgress from '../../components/quiz/QuizProgress'
import { useWrongBook } from '../../hooks/useWrongBook'
import { shouldShowQuizPaywall } from '../../utils/access'
...
const PythonPracticePage: React.FC = () => {
  ...
  const { startQuiz, answerQuestion, stats } = useQuiz()
  ...
  useEffect(() => {
    if (state?.questions?.length) {
      startQuiz(state.questions, 'python')
    } else {
  ...
  useEffect(() => {
    if (shouldShowQuizPaywall(stats.totalAnswered)) {
      setShowPaywall(true)
    }
  }, [stats.totalAnswered])
  ...
  const handleSelectOption = (label: string) => {
    ...
    if (isCorrect) {
      ...
      addWrongAnswer(currentQuestion.id, 'python')
    }

    // 保存答题记录到云端
    answerQuestion(label)
    updateSmartWeights('python', currentQuestion.id, isCorrect, pythonQuestions, PYTHON_BOARD_ORDER)
  }
      ...
      <div className="practice-result">
        ...
        <div className="practice-result__card">
          ...
          <div className="practice-result__actions">
            ...
            <Link to="/quiz/python" className="quiz-btn quiz-btn--secondary" style={{ textDecoration: 'none' }}>
              返回题库
            </Link>
          </div>
          {isTrial && (
            <div className="practice-result__upsell">
              ...
              <Link to="/quiz/python" className="practice-result__upsell-btn">立即解锁</Link>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="practice-page">
      <div className="practice-page__nav">
        <Link to="/quiz/python" className="quiz-btn quiz-btn--ghost">
          &larr; 返回
        </Link>
      ...
      <div className="practice-page__meta">
        ...
        <span className="quiz-card__cat">{PYTHON_CATEGORY_MAP[currentQuestion.category] || currentQuestion.category}</span>
        <span className="practice-page__type">{TYPE_MAP[currentQuestion.type] || currentQuestion.type}</span>
      </div>

      <div className="practice-page__stem">
        <h3 dangerouslySetInnerHTML={{ __html: formatContent(currentQuestion.stem, 'python') }} />
      </div>
        ...
        <>
          <div className="quiz-explanation">
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: formatContent(currentQuestion.explanation, 'python') }}
            />
          </div>
